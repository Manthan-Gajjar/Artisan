...
export function Featured() {
  ...
  return (
    <section id="featured" className="py-20 relative overflow-hidden">
      <div 
        className="absolute -top-24 -right-24 w-64 h-64 rounded-full bg-accent/30 blur-3xl"
        aria-hidden="true"
      />
      ...
      <div className="container mx-auto px-4">
              ...
              <div className="relative aspect-[4/5] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/0 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-6">
                  <div className="text-white transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <p className="text-sm font-medium text-white/70">{artwork.artist}</p>
                    <h3 className="font-playfair text-xl font-medium mt-1">{artwork.title}</h3>
                    <p className="text-sm mt-1 text-white/80">{artwork.medium}</p>
                    <div className="mt-3 pt-3 border-t border-white/20 flex items-center justify-between">
                      <span className="text-white/90 font-medium">{artwork.price}</span>
                      <Button 
                        size="sm" 
                        variant="ghost" 
        ...
        <div className="mt-12 text-center">
          <Button 
            variant="outline" 
            size="lg" 
            className="rounded-full px-8 border-primary/30 hover:bg-background"
          >
            View All Artworks
        ...
        <Dialog open={!!selectedArtwork} onOpenChange={(open) => !open && setSelectedArtwork(null)}>
          <DialogContent className="sm:max-w-[90vw] md:max-w-[85vw] lg:max-w-[1000px] p-0 overflow-hidden bg-background/95 backdrop-blur-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
              {/* Image Section */}
              <div className="relative min-h-[50vh] md:min-h-[70vh] bg-black">
                <Image 
              ...
              <div className="p-6 md:p-8 flex flex-col">
                <DialogHeader>
                  <div className="space-y-1.5">
                    <p className="text-sm font-medium text-muted-foreground">{selectedArtwork.artist}, {selectedArtwork.year}</p>
                    <DialogTitle className="font-playfair text-2xl md:text-3xl font-medium">
                ...
                <div className="mt-6 space-y-4 flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-y-3 text-sm mt-6 pt-4 border-t">
                    <div>
                    ...
                    <div className="col-span-2 mt-2">
                      ...
                      <p className="font-medium text-lg">{selectedArtwork.price}</p>
                    </div>
                  </div>
                </div>
                
                <DialogFooter className="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t">
                  <Button 
